...
use cryptix_miner::xoshiro256starstar::Xoshiro256StarStar;
use cryptix_miner::Worker;
use log::{info, warn};
    ...
    pub fn new(
        ...
        let program = match use_binary {
            true => {
                ...
                match BINARY_DIR.get_file(format!("{}_cryptix-opencl.bin", device_name)) {
                    Some(binary) => {
                        Program::create_and_build_from_binary(&context, &[binary.contents()], "").unwrap_or_else(|e|{
                        //Program::create_and_build_from_binary(&context, &[include_bytes!("../resources/cryptix-opencl-linked.bc")], "").unwrap_or_else(|e|{
