...
use cryptix_miner::PluginManager;
use log::{error, info};
...
use crate::cli::Opt;
use crate::client::grpc::CryptixdHandler;
...
async fn get_client(
    cryptixd_address: String,
    mining_address: String,
    mine_when_not_synced: bool,
    block_template_ctr: Arc<AtomicU16>,
) -> Result<Box<dyn Client + 'static>, Error> {
    if cryptixd_address.starts_with("stratum+tcp://") {
        let (_schema, address) = cryptixd_address.split_once("://").unwrap();
        ...
        .await?)
    } else if cryptixd_address.starts_with("grpc://") {
        Ok(CryptixdHandler::connect(
            cryptixd_address.clone(),
            mining_address.clone(),
            mine_when_not_synced,
            Some(block_template_ctr.clone()),
        )
...
async fn main() -> Result<(), Error> {
    ...
    let (app, mut plugin_manager): (App, PluginManager) = cryptix_miner::load_plugins(Opt::into_app(), &plugins)?;
