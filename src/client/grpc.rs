...
use crate::proto::cryptixd_message::Payload;
use crate::proto::rpc_client::RpcClient;
use crate::proto::{
    GetBlockTemplateRequestMessage, GetInfoRequestMessage, CryptixdMessage, NotifyBlockAddedRequestMessage,
...
type BlockHandle = JoinHandle<Result<(), PollSendError<CryptixdMessage>>>;
...
#[async_trait(?Send)]
impl Client for CryptixdHandler {
...
impl CryptixdHandler {
            ...
            tokio::spawn(async move {
                ReceiverStream::new(recv)
                    .map(|block_seed| match block_seed {
                        FullBlock(block) => CryptixdMessage::submit_block(*block),
    ...
    async fn client_send(&self, msg: impl Into<CryptixdMessage>) -> Result<(), SendError<CryptixdMessage>> {
        self.send_channel.send(msg.into()).await
    }

    async fn client_get_block_template(&mut self) -> Result<(), SendError<CryptixdMessage>> {
    ...
    async fn handle_message(&mut self, msg: Payload, miner: &mut MinerManager) -> Result<(), Error> {
        match msg {
            ...
            Payload::GetInfoResponse(info) => {
                info!("Cryptixd version: {}", info.server_version);
                let cryptixd_version = Version::parse(&info.server_version)?;
                let update_version = Version::parse(VERSION_UPDATE)?;
                match cryptixd_version >= update_version {
            ...
            Payload::NotifyBlockAddedResponse(res) => match res.error {
                None => info!("Registered for block notifications (upgrade your Cryptixd for better experience)"),
